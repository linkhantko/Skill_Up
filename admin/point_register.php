<?php include("header.php");?>
        <div class="main-container">
                <div class="card-box mb-30">
                    <div class="pd-20">
                    <div class="pd-20 card-box mb-30">
                        <div class="clearfix">
                            <div class="pull-left">
                                <h4 class="text-blue h4">Point Register Form</h4>
                            </div>
                            <div class="pull-right">
                                <a href="point" class="btn btn-primary btn-sm scroll-click" role="button"><i class="icon-copy ti-angle-double-left"></i></a>
                            </div>
                        </div>
                    </div>
                    <form action="point_add" method="post" enctype="multipart/form-data">
                        <div class="form-group">
							<label>Point Name</label>
							<input class="form-control" name="pointname" type="text" required>
						</div>
						<div class="form-group">
							<label>Amount</label>
                            <input type="number" class="form-control" name="amount" required>
						</div>
						<div class="form-group">
							<label>Prize</label>
                            <input type="number" class="form-control" name="prize" required>
						</div>
                        <div class="form-group">
                            <div>
                            <button type="submit" name="btnRegister" class="btn btn-primary">
                                Register
                            </button>
                            </div>
                        </div>
                    </form>
                    </div>
                </div>
            </div>
        </div>
        <?php include("footer.php");?>